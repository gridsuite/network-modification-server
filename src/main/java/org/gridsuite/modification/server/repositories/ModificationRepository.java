...
public interface ModificationRepository extends JpaRepository<ModificationEntity, UUID> {
    ...
    @Query(value = "SELECT new ModificationEntity(m.id, m.type, m.date, m.stashed, m.active, m.messageType, m.messageValues) FROM ModificationEntity m WHERE m.group.id = ?1 order by m.modificationsOrder")
